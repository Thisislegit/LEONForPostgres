...
from lero_feature import SampleEntity
...
class LeroNet(nn.Module):
    ...
    def build_trees(self, feature, device=None):
        return prepare_trees(feature, transformer, left_child, right_child, device)
...
class PL_Lero(pl.LightningModule):
    def __init__(self, model, optimizer_state_dict=None, learning_rate=0.001):
        super(PL_Lero, self).__init__()
        self.model = model
        # if self.model.device.type == 'cuda':
        #     self.model.cuda(model.device)
    ...
    def getBatchPairsLoss(self, batch):
        ...
        """
        labels = batch['labels']
        lero_feature1 = batch['lero_feature1']
        lero_feature2 = batch['lero_feature2']
        tree_x1 = self.model.build_trees(lero_feature1, self.device)
        tree_x2 = self.model.build_trees(lero_feature2, self.device)
        y_pred_1 = self(tree_x1)
        y_pred_2 = self(tree_x2)
        diff = y_pred_2 - y_pred_1
        prob_y = F.sigmoid(diff)
        loss_fn = nn.BCELoss()
        loss = loss_fn(prob_y, labels.to(prob_y.dtype).unsqueeze(1))
        # print(loss)
        with torch.no_grad():
            prediction = torch.round(prob_y)
            # print(prediction)
            accuracy = torch.sum(prediction == labels.unsqueeze(1)).item() / len(labels)
    ...
    def training_step(self, batch):
        ...
        self.log_dict({'lero_train_loss': loss, 'lero_train_acc': acc}, on_epoch=True)
        return loss

    def validation_step(self, batch):
        loss, acc = self.getBatchPairsLoss(batch)
        self.log_dict({'lero_val_loss': loss, 'lero_val_acc': acc}, on_epoch=True)
        return loss
