...
LABEL_LIST = ['ActualStartup Time', 'Actual Total Time', 'Actual Self Time']

UNKNOWN_OP_TYPE = "Unknown"
SCAN_TYPES = ["SeqScan", "IndexScan", "IndexOnlyScan", 'BitmapHeapScan']
JOIN_TYPES = ["NestLoop", "HashJoin", "MergeJoin"]
OTHER_TYPES = ['Bitmap Index Scan']
OP_TYPES = [UNKNOWN_OP_TYPE, "Hash", "Material", "Sort", "Aggregate", "IncrementalSort", "Limit"] \
...
class AnalyzeJsonParser(FeatureParser):
    ...
    def extract_feature(self, json_rel) -> SampleEntity:
        ...
        if json_rel['Node Type'] in SCAN_TYPES:
            input_relations.append(json_rel["Relation IDs"])
