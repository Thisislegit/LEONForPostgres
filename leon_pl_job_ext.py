...
if __name__ == '__main__':
    ...
    train_files, training_query, test_files, test_query = envs.load_train_files2(conf['leon']['workload_type'])
    ...
    if pretrain:
        # checkpoint = torch.load("./log/SimModel0206.pth", map_location=DEVICE)
        ...
        # checkpoint = torch.load("./log/DnnModel0312_job_ext.pth", map_location=DEVICE)
    ...
    retrain_count = 3
    min_leon_time = dict()
    logger =  pl_loggers.WandbLogger(save_dir=os.getcwd() + '/logs', name="动态连续等价类完整sql 每次重训leon减参数+dnn job_ext in 202", project=conf['leon']['wandb_project'])
    for key in conf:
    ...
    while ch_start_idx + chunk_size <= len(train_files):
        ...
        if len(dnn_pairs) > min_batch_size:
            ...
            # dnn_model = torch.load("./log/DnnModel0312_job_ext.pth", map_location=DEVICE)
        ...
        if len(train_pairs) > min_batch_size:
            ...
            # model = torch.load("./log/SimModel0206.pth", map_location=DEVICE)
