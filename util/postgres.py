...
def ParsePostgresPlanJson(json_dict):
    ...
    dict_IDToName = {'a1': 'aka_name', 'an': 'aka_name', 'an1': 'aka_name', 
                'aka_t': 'aka_title', 'at': 'aka_title', 
                'ci': 'cast_info', 'chn': 'char_name', 
                'cct1': 'comp_cast_type', 'cct2': 'comp_cast_type', 
                'cn': 'company_name', 'cn1': 'company_name', 'cn2': 'company_name', 
                'ct': 'company_type', 'cc': 'complete_cast', 
                'it': 'info_type', 'it1': 'info_type', 'it2': 'info_type', 'it3': 'info_type', 
                'k': 'keyword', 'kt': 'kind_type', 'kt1': 'kind_type', 'kt2': 'kind_type', 
                'lt': 'link_type', 'mc': 'movie_companies', 'mc1': 'movie_companies', 'mc2': 'movie_companies', 
                'mi': 'movie_info', 'mi_idx': 'movie_info_idx', 'mi_idx1': 'movie_info_idx', 'mi_idx2': 'movie_info_idx', 'miidx': 'movie_info_idx', 
                'mk': 'movie_keyword', 'ml': 'movie_link', 'n': 'name', 'n1': 'name', 
                'pi': 'person_info', 'rt': 'role_type', 't': 'title', 't1': 'title', 't2': 'title'}
    ...
    def _parse_pg(json_dict, select_exprs=None, indent=0):
        ...
        # Special case.
        # pg 返回的plan 中是单个 Relation Name; leon 返回的 plan 中是多个 Relation IDs
        if 'Relation IDs' in json_dict:
            curr_node.table_name = json_dict['Relation IDs']
            if ' ' not in json_dict['Relation IDs']:
                curr_node.table_name = dict_IDToName[json_dict['Relation IDs']]
            # print("--- json_dict['Relation IDs'] ---", json_dict['Relation IDs'])
            # curr_node.table_name = dict_IDToName[json_dict['Relation IDs']]
            curr_node.table_alias = json_dict['Relation IDs']
        if 'Relation Name' in json_dict: # 
            # print("--- json_dict['Relation Name'] ---\n", json_dict['Relation Name'])
