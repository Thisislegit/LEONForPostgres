...
class QueryFeaturizer(Featurizer):
    ...
    def __call__(self, node):
        ...
        for rel_id in joined:
            print("--- rel_id ---", rel_id)
            print("--- np.where(self.workload_info.rel_ids == rel_id) ---", np.where(self.workload_info.rel_ids == rel_id))
            print("--- np.where(self.workload_info.rel_ids == rel_id)[0] ---", np.where(self.workload_info.rel_ids == rel_id)[0])
